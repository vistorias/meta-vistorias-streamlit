...
st.info("👋 **Bem-vindo(a) ao Painel de Acompanhamento de Metas!**\n\n"
        "Aqui você pode acompanhar em tempo real a performance das unidades e identificar oportunidades de melhoria com base nas metas do mês. "
        "Use os filtros à esquerda para ajustar os dados conforme o período desejado.")
...
# 📄 Carregar planilha
sheet = client.open_by_key("1ooMhPk-R-Etzut4BHkxCTgYZx8ftzHzDIhyXuS9TLGo").sheet1
data = sheet.get_all_records()
df = pd.DataFrame(data)

# ⚙️ Limpeza e prevenção de erros
...
df['%_190'] = pd.to_numeric(df['%_190'], errors='coerce').fillna(0).astype(int)
...
for _, row in df_filtrado.iterrows():
    ...
    ticket = round(row['ticket_medio_real'], 2) if not pd.isna(row['ticket_medio_real']) else 0
